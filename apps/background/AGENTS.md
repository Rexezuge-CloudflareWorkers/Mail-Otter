...
- Shared scheduled bases: `IScheduledTask` (Template Method + `createApplicationRun` Builder), `BaseDriveSyncTask` (both Drive sync tasks), `RepositoryHelper.pruneInBatches` (`pruneInBatches`, `computeUnixCutoffSeconds`, `computeDateCutoffIso`, `DEFAULT_PRUNE_BATCH_SIZE` in `backend-data/utils`).
