...
class DigestEmailUtil {
  ...
  private static buildPackagesSection(actions: EmailAction[], locale?: string | null): string {
    ...
    const rows = actions.map((a) => {
      ...
      const eta = syncStatus?.expectedDelivery ? `<br><span style="color:#666;font-size:13px">${EmailContentUtil.sanitizeHtml(formatBackendString(strings.digest.expectedLabel, { value: syncStatus.expectedDelivery }))}</span>` : '';
      const carrier = payload.carrier ? ` (${EmailContentUtil.sanitizeHtml(payload.carrier)})` : '';
      const link = payload.trackingUrl ? ` <a href="${EmailContentUtil.sanitizeHtml(payload.trackingUrl)}" style="color:#2563eb">${EmailContentUtil.sanitizeHtml(strings.digest.trackLink)}</a>` : '';
  ...
  private static buildBillsSection(actions: EmailAction[], locale?: string | null): string {
    ...
    const rows = actions.map((a) => {
      ...
      const dueDate = payload.dueDate ? ` — ${EmailContentUtil.sanitizeHtml(strings.digest.duePrefix)} ${EmailContentUtil.sanitizeHtml(payload.dueDate)}` : '';
      const amount = payload.amount ? ` ${EmailContentUtil.sanitizeHtml(payload.currency || '')}${EmailContentUtil.sanitizeHtml(payload.amount)}` : '';
      const link = payload.paymentUrl ? ` <a href="${EmailContentUtil.sanitizeHtml(payload.paymentUrl)}" style="color:#2563eb">${EmailContentUtil.sanitizeHtml(strings.digest.payLink)}</a>` : '';
  ...
  private static buildAppointmentsSection(actions: EmailAction[], locale?: string | null): string {
    ...
    const rows = actions.map((a) => {
      ...
      const location = payload.location ? ` ${EmailContentUtil.sanitizeHtml(strings.digest.atPrefix)} ${EmailContentUtil.sanitizeHtml(payload.location)}` : '';
